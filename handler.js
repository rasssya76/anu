...
export async function participantsUpdate({ id, participants, action }) {
    ...
    await conn.sendButtonImg(id, pp, text, me, action == 'add' ? 'Wlcm' : 'Sayonaraaa', action === 'add' ? '.intro' : 'ImYanXiao', { mentions: [user]})       
                    }
                }
            }
        break*/
 ...
 /* conn.sendButtonDoc(id, wm, text, action == 'add' ? 'ᴡᴇʟᴄᴏᴍᴇ' : 'sᴀʏᴏɴᴀʀᴀᴀ', action === 'add' ? '.intro' : 'Aʟᴅɪ X Aɪsʏᴀʜ', fkontak, { contextInfo: { externalAdReply: { showAdAttribution: true,
  ...
  conn.sendButtonImg(id, await(await fetch(action === 'add' ? wel : lea)).buffer(), 'Group Messege', text, action == 'add' ? 'ᴡᴇʟᴄᴏᴍᴇ' : 'sᴀʏᴏɴᴀʀᴀᴀ', action === 'add' ? '.intro' : 'Aʟᴅɪ X Aɪsʏᴀʜ', { contextInfo: { externalAdReply: { showAdAttribution: true,
    mediaUrl: 'https://instagram.com',
    mediaType: 2, 
    description: `https://chat.whatsapp.com/Bd2mN13f4QV7Mu1vtZ0NuP`,
    title: "Jᴏɪɴ Sɪɴɪ Cᴜʏ",
